...
def get_modified_terminal_element(mt):
    ...
    return element, local_derivatives, fc
    ...
    for mt in modified_terminals:
        ...
        if name in tables:
            ...
            if entitytype == "facet":
                    ...
                    for ref in range(2):
                        new_table.append(get_ffcx_table_values(
                            permute_quadrature_interval(quadrature_rule.points, ref),
                                         cell, integral_type, element, local_derivatives, flat_component))
                ...
                tbl = get_ffcx_table_values(quadrature_rule.points, cell,
                                            integral_type, element, local_derivatives, flat_component)
            # Clean up table
        ...
        dofmap = tuple(cell_offset + offset + i * stride for i in range(num_dofs))
