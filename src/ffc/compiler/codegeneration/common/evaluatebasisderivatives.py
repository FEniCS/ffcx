...
def generate_map(element, format):
    ...
    eta_triangle = [(eta_triangle_snip) %(format_floating_point(FFC_EPSILON))]

    eta_tetrahedron = [(eta_tetrahedron_snip) %(format_floating_point(FFC_EPSILON),\
                       format_floating_point(FFC_EPSILON))]

    # List of coordinate declarations
    coordinates = [((format_float + "x"), format_coordinates(0)), \
    ((format_float + "y"), format_coordinates(1)), ((format_float + "z"),\
    format_coordinates(2))]
...
def dot_product(element, format):
    ...
    code += [(format["comment"]("Compute value(s)"))]
    ...
    if (len(shape) == 2):
        ...
        code += [(format["add equal"]("*values","coefficients0[i][j]*basisvalues[j]"))]
    ...
    # Vector valued basis element [Raviart-Thomas, Brezzi-Douglas-Marini (BDM)]
        ...
        code += [(format["add equal"]("values[%d]" %(i),\
...
def generate_cases(element, format):
    ...
    code += [(format["comment"]("Switch for each of the unique sub elements"))]
...
def dof_map(elements, format):
    ...
    code = [(format["comment"]("Map basis function to local basisfunction"))]
    code += [(evaluate_basis_dof_map) % len(elements)]
