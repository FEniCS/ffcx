...
def __generate_evaluate_dof(element, dof_map, cell_dimension):
    """Generate code for finite_element::evaluate_dof. This function is
    only implemented for Lagrange elements and not pregenerated by the
    ...
    X = "{" + ", ".join(["{" + ", ".join(["%.15g" % x for x in c]) + "}" for c in cs]) + "}"

    # Get dof components
    cs = dof_map.dof_components()
    components = "{" + ", ".join(["%d" % c for c in cs]) + "}"
    
    # Pick template
    if cell_dimension == 2:
        evaluate_dof = evaluate_dof_2D
    else:
        evaluate_dof = evaluate_dof_3D
    ...
    # Generate code
    return evaluate_dof % (num_values, dof_map.local_dimension(), X, dof_map.local_dimension(), components)
