...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "hnr:t:", \
        ["help", "new_references", "representation=", "tolerance="])
    ...
    # Clean FFC cache
    # FIXME: it might be possible to just delete the forms cached from the demo directory
    # Ask user if he/she wants to continue
    cont = raw_input("\n*** Warning! This script will delete all forms in cache! ***\nDo you wish to continue? (yes/no): ")
    affirmative = ["y", "yes", "Y", "Yes"]
    if not cont in affirmative:
        print "Aborting"
        return 2
    print "\nDeleting forms in cache"
    print "====================================================================\n"
    time.sleep(2)
    system("instant-clean")
    print "\n====================================================================\n"

    test_options = {"tolerance":tolerance, "new_references":new_references, "form_files":args}
...
def run_tests(test_options):
    ...
    do_not_compile = ["Projection.form"]
    new_files = []
...
def verify_form(form_file, form_type, form, forms_not_compiled_ok, forms_not_compared_ok, ok_compile, norm_ok, test_options):
    ...
    try:
        ...
        opt = {"representation":test_options["representation"]}
...
def compute_norm(compiled_form, form_data, file_name, test_options):
    ...
    macro_w = [0]*form_data.num_coefficients
    for i in range(form_data.num_coefficients):
