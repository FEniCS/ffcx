...
This function returns the number of dofs associated with a single facet of a
...
This function returns the number of degress of freedom associated
with a single mesh entity of dimension \texttt{d} in a cell, which
are not associated with any entities of dimension < d.
...
See also \texttt{ufc::finite\_element::tabulate\_entity\_dofs}.
...
This includes dofs that are associated with lower dimensional entities,
ie the vertices of an edge in 2D and both vertices and edges of a face in 3D.
The dofs returned by this function should not be used for connectivity information,
use \texttt{ufc::dof\_map::tabulate\_entity\_dofs} instead for that purpose.
...
and not associated with any entities of dimension < d.
The size of the output array \texttt{dofs} should be equal to
the value returned by \texttt{num\_entity\_dofs(d)}.
...
To retrieve all dofs related to a facet for applying Dirichlet boundary conditions,
see \texttt{ufc::dof\_map::tabulate\_facet\_dofs}.
