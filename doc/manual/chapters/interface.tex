...
which define the central interface class \texttt{ufc::form} and a
...
The interface is presented below in the same order in which it is
...
dimension two, and tetrahedron and hexahedron have dimension three.
...
should contain the $x$, $y$, \ldots coordinates of the first vertex
etc.
...
The input of \texttt{evalute} are the coordinates of a point in a cell
and the UFC view of the cell containing the point.
...
two incident cells. Thus, when the facet is an edge of triangle, each
...
arguments may be an integer betwen zero and three (0, 1, 2, 3).
...
coefficients~$n$ has $r + n$ arguments, such that this function
