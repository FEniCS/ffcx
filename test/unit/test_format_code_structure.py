...
def test_format_basics():
    # Reproduce a string
    assert format_code_structure("string") == "string"

    # Insert keywords in string using <2.6 syntax
    code = format_code_structure("%(string)s %(other)s", keywords=dict(string='Hello', other='World'))
    assert code == "Hello World"

    # Basic strings with indentation
    assert format_code_structure("string", 1) == "    string"
    assert format_code_structure("string", 2, indentchar='xy') == "xyxystring"

    # Multiline strings with indentation
    assert format_code_structure("fee\nfie\nfoe", level=1, indentchar='q') == "qfee\nqfie\nqfoe"

    # One item lists are the same as strings
    assert format_code_structure(["hello"]) == "hello"
    assert format_code_structure(["hello"], 1) == "    hello"
    assert format_code_structure(["hello"], 2, indentchar='xy') == "xyxyhello"

    # Tuples are concatenated directly
    assert format_code_structure(("hello", "world")) == "helloworld"
    assert format_code_structure(("hello", "world"), 1) == "    helloworld"
    code = format_code_structure(("hello", "world"), 2, indentchar='xy')
    assert code == "xyxyhelloworld"

    # Lists are joined with newlines
    assert format_code_structure(["hello", "world"]) == "hello\nworld"
    assert format_code_structure(["hello", "world"], 1) == "    hello\n    world"
    assert format_code_structure(["hello", "world"], 2, indentchar='xy') == "xyxyhello\nxyxyworld"

def test_format_blocks():
    # Strings and lists can be put in Indented containers
    assert format_code_structure(Indented("hei")) == "    hei"
    code = format_code_structure(Indented(["hei", Indented("verden")]), indentchar='z')
    assert code == "zhei\nzzverden"

    # A Block is an indented body with brackets before and after
    code = format_code_structure(["{", Indented("fee\nfie\nfoe"), "}"], indentchar='\t')
    assert code == "{\n\tfee\n\tfie\n\tfoe\n}"
    code = format_code_structure(Block("fee\nfie\nfoe"), indentchar='\t')
    assert code == "{\n\tfee\n\tfie\n\tfoe\n}"
    # A Namespace is a 'namespace foo' line before a block
    code = format_code_structure(Namespace("bar", "fee\nfie\nfoe"), indentchar='\t')
    assert code == "namespace bar\n{\n\tfee\n\tfie\n\tfoe\n}"

    # Making a for loop
    code = format_code_structure(["for (iq...)", Block("foo(iq);")])
    assert code == "for (iq...)\n{\n    foo(iq);\n}"
    # Making a do loop
    code = format_code_structure(["iq = 0;", "do", (Block("foo(iq);"), " while (iq < nq);")])
    assert code == "iq = 0;\ndo\n{\n    foo(iq);\n} while (iq < nq);"

def test_format_keywords():
    # Making a for loop with keywords
    code = format_code_structure(["for (%(i)s=0; %(i)s<n; ++%(i)s)", Block("foo(%(i)s);")], keywords={'i': 'k'})
    assert code == "for (k=0; k<n; ++k)\n{\n    foo(k);\n}"
    format_code_structure(WithKeywords(["for (%(i)s=0; %(i)s<n; ++%(i)s)",
                                        Block("foo(%(i)s);")], keywords={'i': 'k'}))
    assert code == "for (k=0; k<n; ++k)\n{\n    foo(k);\n}"

    # Formatting the same code with different keywords using WithKeywords
    tmp = ['%(a)s', '%(b)s']
    code = format_code_structure([WithKeywords(tmp, keywords={'a': 'a', 'b':'b'}),
                                WithKeywords(tmp, keywords={'a': 'x', 'b':'y'})])
    assert code == "a\nb\nx\ny"

def test_format_class():
    # Making a class declaration
    assert format_code_structure(Class('Car')) == 'class Car\n{\n};'
    code = format_code_structure(Class('Car', superclass='Vehicle'))
    assert code == 'class Car: public Vehicle\n{\n};'
    code = format_code_structure(Class('Car', public_body='void eval()\n{\n}'))
    assert code == 'class Car\n{\npublic:\n    void eval()\n    {\n    }\n};'
    code = format_code_structure(Class('Car', protected_body='void eval()\n{\n}'))
    assert code == 'class Car\n{\nprotected:\n    void eval()\n    {\n    }\n};'
    code = format_code_structure(Class('Car', private_body='void eval()\n{\n}'))
    assert code == 'class Car\n{\nprivate:\n    void eval()\n    {\n    }\n};'

def test_format_template_argument_list():
    def t(args, mlcode, slcode):
        code = format_code_structure(TemplateArgumentList(args, False), indentchar=' ')
        assert code == slcode
        code = format_code_structure(TemplateArgumentList(args, True), indentchar=' ')
        assert code == mlcode
    t(('A',), '<\n A\n>', '<A>')
    t(('A', 'B'), '<\n A,\n B\n>', '<A, B>')

def test_format_templated_type():
    code = format_code_structure(Type('Foo'))
    assert code == 'Foo'
    code = format_code_structure(Type('Foo', ('int', '123')))
    assert code == 'Foo<int, 123>'
    code = format_code_structure(Type('Foo', ('int', Type('Bar', ('123', 'float')))))
    assert code == 'Foo<int, Bar<123, float> >'

def test_format_typedef():
    assert format_code_structure(TypeDef('int', 'myint')) == 'typedef int myint;'
    code = format_code_structure(TypeDef(Type('Foo', ('int', Type('Bar', ('123', 'float')))), 'Thing'))
