...
class ArrayDecl(Code):
    ...
    def format(self, level, indentchar, keywords):
        ...
        if self.values is None:
            ...
            r = len(self.sizes)
            assert r > 0
            assert len(self.values) == self.sizes[0]
            if r == 1:
                valuescode = tuple(build_initializer_list(self.values))
            elif r == 2:
                assert len(self.values[0]) == self.sizes[1]
                inner = []
                for i0 in range(self.sizes[0]):
                    inner.append( tuple(build_initializer_list(self.values[i0])) )
                valuescode = ["{", Indented([build_separated_list(inner, ","), "}"])]
            elif r == 3:
                assert len(self.values[0]) == self.sizes[1]
                assert len(self.values[0][0]) == self.sizes[2]
                outer = []
                for i0 in range(self.sizes[0]):
                    inner = []
                    for i1 in range(self.sizes[1]):
                        inner.append( tuple(build_initializer_list(self.values[i0][i1])) )
                    outer.append(Indented(["{", build_separated_list(inner, ","), "}"]))
                valuescode = ["{", Indented([build_separated_list(outer, ","), "}"])]
            else:
                error("TODO: Make recursive implementation of array initialization.")
...
class ArrayAccess(Code):
    def __init__(self, arraydecl, indices):
        ...
        if len(indices) != len(self.arraydecl.sizes):
            raise ValueError("Invalid number of indices.")
        if any(isinstance(i, int) and isinstance(d, int) and i >= d
               for i,d in zip(indices, self.arraydecl.sizes)):
    ...
    def format(self, level, indentchar, keywords):
        ...
        code = (self.arraydecl.name, brackets)
        return format_code_structure(code, level, indentchar, keywords)
