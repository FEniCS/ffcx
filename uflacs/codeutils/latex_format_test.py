...
def test_latex_formatting():
    t = LatexFormatterTest()
    return t.test_latex_formatting()

class LatexFormatterTest(object):
    ...
    def assertEqual(self, a, b):
        if a == b:
            self.teststats.successes += 1
        else:
            self.teststats.fails += 1
            print "FAIL self.assertLatexEqual:"
            print "Actual:   '%s'" % r
            print "Expected: '%s'" % code
            print

    def test_latex_formatting(self):
        for fn in vars(type(self)).iterkeys():
            if fn.startswith('test_') and fn != 'test_latex_formatting':
                f = getattr(self, fn)
                if callable(f):
                    f()
        # Report final test results
        print self.teststats
        return self.teststats.fails
