...
def compile_expression_partitions(expression, form_argument_mapping, parameters):
    ...
    # FIXME: Apply form_argument_mapping already at this point?
    ...
    if 0: # TODO: Use this!
        ...
        # TODO:
        # - find dofrange for each argument_factors value
        #dofranges[np][iarg] = [...]

        # TODO:
        # - find dofblock for each argument_factorization item
        #dofblocks[np] = [...]
        ...
        # Build numbering of partitions
        partition_labels = {}
        # partition_labels["piecewise"] = partition of expressions independent of quadrature and argument loops
        p = 1
        ...
        for np in num_points:
            # partition_labels["varying"][np] = partition of expressions dependent on np quadrature but independent of argument loops
            partition_labels["varying"][np] = p; p += 1

            # TODO: Add partition labels for conditional cases? Maybe nested label tuples? This is not thought through...
            ...
            # partition_labels["argument"][np][iarg][dofrange] = partition depending on this dofrange of argument iarg
            ...
            # partition_labels["integrand"][np][dofrange] = partition depending on this dofrange of argument iarg
        ...
        # TODO:
        # - reorder vertices grouped by partition number
        # - change mark_partitions to use this numbering instead
        # - change code generation to use this numbering instead
    ...
    partitions = mark_partitions(SV, active, dependencies, form_argument_mapping) # FIXME: Apply form_argument_mapping earlier
...
def generate_expression_code(ir, cell, form_argument_mapping, object_names,
    ...
    dependency_handler = DependencyHandler(ir["terminals"], cell, form_argument_mapping, object_names)

    # This formatter is a multifunction implementing target specific formatting rules
    language_formatter = create_language_formatter(dependency_handler, ir)

    # Create a formatter for blocks of statements
    statement_formatter = create_statement_formatter(dependency_handler, ir)
