...
class DolfinExpressionStatementFormatter(object):
    ...
    # --- Using default rules for geometry variables
    ...
    def define_argument_loop_vars(self, argument_count):
        arg = self.arguments[argument_count]
        base_name = self.argument_names[arg]
        rnames = {None:"", "+": "_p", "-": "_m"}

        code = []

        # Generate code for all needed derivatives of arguments with this count
        for o in self.terminals:
            (t, ngrads, r) = analyse_terminalish(o)
            if isinstance(t, Argument):
                count = t.count() # FIXME: Is this with or without mapping?
                if count == argument_count:
                    # TODO: Consider self.needed to avoid computing unneeded components.
                    sh = o.shape()

                    # TODO: Share name construction code with form_argument() below
                    derivative_prefix = 'd%d_' % (ngrads,) if ngrads else "v_"
                    declname = derivative_prefix + base_name + rnames[r]
                    if sh:
                        # TODO: Flat or nested array?
                        declname += "[%d]" % (product(sh),)

                    decl = "double %s = { ... };" % (declname,)
                    code += [decl]
...
class DolfinExpressionFormatter(object):
    # TODO: This class uses: self.coefficient_names, self.argument_names, self.require()
    def __init__(self):
        pass
...
# TODO: Split up usage of these two classes
class DolfinExpressionFormatter(CppDefaultFormatter, DolfinExpressionStatementFormatter, DolfinExpressionLanguageFormatter):
    ...
    def __init__(self, object_names):
        ...
        DolfinExpressionStatementFormatter.__init__(self)
        DolfinExpressionLanguageFormatter.__init__(self)
