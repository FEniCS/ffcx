...
def element_table_access(ir, num_points, component, derivatives, restriction, element, idof):
    ...
    code = "%s[%s][%s]" % (unique_table_name, names.iq, idof)
...
class FFCStatementFormatter(object):
    ...
    def define_coord_vars(self):
        ...
        if self._enable_accumulation:
            ...
            code += ["const double %s = %s[%s];" % (names.qw, names.weights, names.iq)]
    ...
    def define_piecewise_coefficients(self):
        ...
        code = ["// %s = %s[%d][:]" % (dh.coefficient_names[c], names.w, i)
    ...
    def _define_coord_dependent_coefficient(self, w, reqdata):
        ...
        import ffc
        ffc_element = ffc.fiatinterface.create_element(element) # TODO
        ...
        for (c,d,r), wname in reqdata.iteritems():
            r_offset = r_offsets[r]

            if wsh:
                # Map component to flat index
                vi2si, si2vi = build_component_numbering(wsh, element.symmetry())
                c = vi2si[c] # FIXME: Apply this numbering in require? Getting duplicate [0,1],[1,0]!
            else:
                assert c == ()
                c = 0
            ...
            if d:
                ...
                dbftable = [[element_table_access(self._ir, self._num_points, c, ld, r, element, i)
                ...
                if 1:
                    ...
                    print
                if non_zeros:
                    lower = min(non_zeros[1])
                    upper = max(non_zeros[1])
                    assert isinstance(non_zeros[0], int)
                    assert isinstance(c,int), "FIXME: What kind of component is this really?"
                    assert non_zeros[0] == c
                    assert len(non_zeros[1]) == upper - lower + 1
                else:
                    lower = 0
                    upper = fe_dim
                    assert c == 0, "FIXME: What kind of component is this really?"

                offset = lower + r_offset

                # Format loop accumulating product of dofs and table values
                # TODO: Use formatting utils for cleaner code formatting?
                accum = "%s += %s;"
                prod = "%s * %s"
                wexpr = "%s[%s][%s + %s]" % (names.w, wc, names.ic, offset)
                feexpr = "%s[%s][%s]" % (unique_table_name, names.iq, names.ic)
                w_fe = prod % (wexpr, feexpr)
                body = accum % (wname, w_fe)

                code.append("double %s = 0.0;" % (wname,))
                code.append(ForRange(names.ic, 0, upper-lower, body=body))
    ...
    def output_variable_names(self, num_variables):
        return ["%s[%d]" % (names.A, i) for i in xrange(num_variables)]
...
class FFCLanguageFormatter(MultiFunction, CppFormatterRulesCollection):
    ...
    def _piecewise_constant_coefficient(self, o, component, derivatives, restriction):
        ...
        return "%s[%d][%d]" % (names.w, o.count(), comp)
