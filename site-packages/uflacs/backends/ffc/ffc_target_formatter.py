...
fmt_access = "%s[%s]"
fmt_access2 = "%s[%s][%s]"
restriction_postfix = { "+": "_0", "-": "_1", None: "" } # TODO: Use this wherever we need it?
...
def get_inline_mapping_row(element, gd, tdim, r):
    # TODO: Pick right mapping and verify indexing
    if 1:
        mapping_row = [fmt_access % ("K", "%d*%d + %d" % (gd, tdim, ld)) for ld in range(tdim)]
    else:
        mapping_row = [fmt_access % ("J", "%d*%d + %d" % (ld, gdim, gd)) for ld in range(tdim)]
    return mapping_row
    ...
    def _define_coord_dependent_coefficient(self, w, reqdata):
        ...
        for (c,r) in sorted(crdata.keys()):
            ...
            if value_varname:
                ...
                (unique_table_name, non_zeros, zeros, ones) = \
                  get_element_table_data(self._ir, self._num_points, element, c, (), r)
            ...
            for order in sorted(derivative_orders):
                ...
                if c != ():
                    locgradname += "_c%d" % c
                locgradname += restriction_postfix[r]
            ...
            for d, dvalue_varname in sorted(derivative_varnames):
                ...
                mapping_row = get_inline_mapping_row(element, gd, tdim, r)
        ...
        if reqdata:
            ...
            for (c,r) in sorted(crdata.keys()):
                ...
                for d, dvalue_varname in sorted(derivative_varnames):
                    ...
                    # Get list with direct access to local gradient values
                    local_grad = [element_table_access(self._ir, self._num_points, c, (ld,), r, element, idof)
                                  for ld in range(tdim)]
...
class FFCLanguageFormatter(MultiFunction, CppFormatterRulesCollection):
    ...
    def _computed_form_argument_name(self, o, component, derivatives, restriction, basename):
        ...
        code += restriction_postfix[restriction]
    ...
    def argument(self, o, component=(), derivatives=(), restriction=None):
        ...
        if derivatives:
            ...
            unique_table_name, non_zeros, zeros, ones = \
              get_element_table_data(self._ir, self._num_points, element, comp, derivatives, restriction)
