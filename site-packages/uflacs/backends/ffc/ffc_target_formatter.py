...
class FFCStatementFormatter(object):
    ...
    def _define_coord_dependent_coefficient(self, w, reqdata):
        ...
        for ((c,d,r),varname) in reqdata.iteritems():
            ...
            if d:
                ...
                if key not in ddata:
                    ddata[key] = []
                    dcount[key] = set()
                ddata[key].append((d,varname))
                dcount[key].add(len(d))
            else:
                vdata += [(c,r,varname)]

        # Body of merge coefficient loops for values and local derivatives:
        body = []

        # Compute coefficient values
        for c,r,varname in vdata:
            ...
            code.append("double %s = 0.0;" % (varname,))
            ...
            body.append(fmt_accum % (varname, w_x_fe))

        # Compute local coefficient derivatives
        for (c,r) in sorted(ddata.keys()):
            # Compute local gradient (and eventually local hessian etc.) for component c,r
            for order in sorted(dcount[(c,r)]):
        ...
        for (c,r) in sorted(ddata.keys()):
            for d, varname in ddata[(c,r)]:
                uflacs_assert(len(d) == 1, "Higher order derivatives not yet implemented.") # TODO

                # FIXME: Map derivatives with K (or J^T) to get the global derivative direction d and store in varname
                pass # FIXME: Apply mapping and store in varname
