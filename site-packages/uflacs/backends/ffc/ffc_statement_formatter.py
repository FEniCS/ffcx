...
from uflacs.algorithms.table_utils import derivative_listing_to_counts
...
def get_element_table_data(ir, num_points, element, entity, component, derivatives):
    ...
    element_table_name = _generate_psi_name(element_number, ir["entitytype"], entity, component, derivative_counts)
...
def element_table_access(ir, num_points, element, entity,
                         component, derivatives, idof):
    unique_table_name, non_zeros, zeros, ones = \
        get_element_table_data(ir, num_points, element, entity, component, derivatives)
...
def get_inline_mapping_row(element, gd, tdim, gdim, r):
    # TODO: Pick right mapping and verify indexing
    if 1:
...
class FFCStatementFormatter(object):
    ...
    def define_piecewise_coefficients(self):
        ...
        for i, c in enumerate(dh.coefficients):
            c = dh.form_argument_mapping.get(c,c)
            if c.is_cellwise_constant():
                cc = c.count()
                assert cc == i, "When this fails, need to take a good look at use of form argument mappings and coefficients."
    ...
    def _define_coord_dependent_coefficient(self, w, reqdata):
        ...
        def _compute_dof_range(non_zeros, fe_dim, r):
            # Get bounds on dof index loop
            if non_zeros:
                lower, upper = min(non_zeros[1]), max(non_zeros[1])+1
                uflacs_assert(list(non_zeros[1]) == list(range(lower,upper)),
                              "Assumption of contiguous nonzero dof indices failed!")
            else:
                lower, upper = 0, fe_dim
            # Add restriction offset and lower dof index bound
            dof_offset = lower + r_offsets[r]
            num_dofs = upper - lower
            return lower, upper, num_dofs, dof_offset
        ...
        for (c,r) in sorted(crdata.keys()):
            ...
            if value_varname:
                ...
                (unique_table_name, non_zeros, zeros, ones) = \
                  get_element_table_data(self._ir, self._num_points, element, entity, c, ())

                # Declare variable for coefficient value initialized to zero
                code.append("double %s = 0.0;" % (value_varname,))

                # Get bounds on dof index loop
                lower, upper, num_dofs, dof_offset = _compute_dof_range(non_zeros, fe_dim, r)
            ...
            for order in sorted(derivative_orders):
                ...
                for ld in range(tdim):
                    ...
                    (unique_table_name, non_zeros, zeros, ones) = \
                      get_element_table_data(self._ir, self._num_points, element, entity, c, (ld,))

                    # Get bounds on dof index loop
                    lower, upper, num_dofs, dof_offset = _compute_dof_range(non_zeros, fe_dim, r)

                    # FIXME: Make ffc capable of removing contiguous trailing zeros but keeping contiguous dofs in tables,
                    #        allowing dof ranges to work here! Currently eliminate zeros is disabled in the uflacsrepr in ffc.

                    num_dofs_set.add(num_dofs)
                    print
                    print ld
                    print non_zeros
                    print
                    ...
                    # Format statement accumulating product of dofs and table values
                    wexpr = fmt_access2 % (names.w, wc, "%s + %s" % (names.ic, dof_offset))
                    dfeexpr = fmt_access2 % (unique_table_name, names.iq, names.ic)
                    ...
                    code.append(ForRange(names.ic, 0, num_dofs, body=body))
                #print num_dofs_set
                #uflacs_assert(len(num_dofs_set) == 1, "Assumption necessary for merging loops failed!")

            # Map local derivatives with K (or J^T) to get the global derivatives
            for d, dvalue_varname in sorted(derivative_varnames):
                uflacs_assert(len(d) == 1, "Higher order derivatives not yet implemented.") # TODO
                gd = d[0]
                ...
                # Map local gradient with row of K (or J^T) to get the global derivative direction d
                dvalue_expr = " + ".join(fmt_prod % (mapping_row[ld], local_grad[ld]) for ld in range(tdim))

                # Declare variable for coefficient derivative value initialized with inline mapping expression
    ...
    def define_argument_loop_vars(self, argument_count):
        ...
        _v = dh.arguments[argument_count]
        v = dh.form_argument_mapping[_v]
        ...
        if reqdata:
            ...
            for (c,r) in sorted(crdata.keys()):
                ...
                uflacs_assert(tuple(derivative_orders) == (1,), "Higher order derivatives not yet implemented.") # TODO
                ...
                for d, dvalue_varname in sorted(derivative_varnames):
                    ...
                    # Get list with direct access to local gradient values
                    local_grad = [element_table_access(self._ir, self._num_points, element, entity,
                                                       c, (ld,), idof)
                                  for ld in range(tdim)]
