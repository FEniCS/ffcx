...
from uflacs.algorithms.table_utils import generate_psi_table_name
...
def format_entity_name(entitytype, r):
    if entitytype == "cell":
        ...
        entity = "facet" + {"+":"0", "-":"1", None:""}[r]
    elif entitytype == "vertex":
        entity = "vertex"
    return entity

def get_element_table_data(ir, entitytype, num_points, element, flat_component, derivatives):
    ...
    element_table_name = generate_psi_table_name(element_counter, flat_component, derivative_counts, entitytype)

    unique_table_name, begin, end = ir["table_ranges"][element_table_name]

    return unique_table_name, begin, end

def format_element_table_access(ir, entitytype, num_points, element, flat_component, derivatives, entity, idof):
    unique_table_name, begin, end = get_element_table_data(ir, entitytype, num_points, element, flat_component, derivatives)
    return fmt_access3 % (unique_table_name, entity, names.iq, "%s + %s" % (begin, idof))
    ...
    def _define_coord_dependent_coefficient(self, w, reqdata):
        ...
        for (c,r) in sorted(crdata.keys()):
            ...
            if value_varname:
                ...
                unique_table_name, begin, end = get_element_table_data(self._ir, self._entitytype, self._num_points, element, c, ())
                ...
                feexpr = fmt_access3 % (unique_table_name, entity, names.iq, names.ic)
            ...
            for order in sorted(derivative_orders):
                ...
                for ld in range(tdim):
                    ...
                    unique_table_name, begin, end = get_element_table_data(self._ir, self._entitytype, self._num_points, element, c, (ld,))
                    ...
                    # Format statement accumulating product of dofs and table values
                    wexpr = fmt_access2 % (names.w, wc, "%s + %s" % (names.ic, dof_offset))
                    dfeexpr = fmt_access3 % (unique_table_name, entity, names.iq, names.ic)
        ...
        if reqdata:
            ...
            for (c,r) in sorted(crdata.keys()):
                ...
                for d, dvalue_varname in sorted(derivative_varnames):
                    ...
                    # Get list with direct access to local gradient values
                    local_grad = [format_element_table_access(self._ir, self._entitytype, self._num_points,
                                                              element, c, (ld,), entity, idof)
                                  for ld in range(tdim)]
