...
def format_element_table_access(ir, entitytype, num_points, element,
    ...
    if preserved:
        ...
        dof = "%s + %s" % (begin, idof)
    return fmt_access3 % (unique_table_name, entity, names.iq, dof)
...
class FFCStatementFormatter(object):
    ...
    def define_piecewise_geometry(self):
        ...
        # TODO: Seed this from terminal data and extend with dependency dict
        needed = {}
        needed["J"] = ("",)
        needed["detJ"] = ("",)
        needed["K"] = ("",)
        ...
        for r in needed.get("detJ", ()):
            code.extend(generate_jacobian_determinants_snippets(self._cell, r))
        ...
        if self._entitytype == "cell":
            for r in needed.get("det", ()):
                code.extend(generate_cell_scaling_factor_snippets(self._cell))
        elif self._entitytype == "facet":
            for r in needed.get("det", ()):
                code.extend(generate_facet_scaling_factor_snippets(self._cell, r))
            for r in needed.get("facet_area", ()):
                code.extend(generate_facet_area_snippets(self._cell, r))
            for r in needed.get("n", ()):
                code.extend(generate_facet_direction_snippets(self._cell, r))
                code.extend(generate_facet_normal_snippets(self._cell, r))
        ...
        for r in needed.get("volume", ()):
            code.extend(generate_cell_volume_snippets(self._cell, r))
    ...
    def define_coord_vars(self):
        ...
        if self._enable_accumulation:
            ...
            # TODO: Use restriction in names for 'det', 'qw', 'D'?
            code += ["const double %s = %s * %s;" % (names.D, names.qw, names.det)]
