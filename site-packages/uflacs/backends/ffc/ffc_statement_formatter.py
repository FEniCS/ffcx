...
class FFCStatementFormatter(object):
    ...
    def define_piecewise_geometry(self):
        ...
        if self._enable_quadrature_rule:
            weights = langfmt.precision_float(self._weights)
    ...
    def define_coord_vars(self):
        ...
        if self._enable_accumulation:
            ...
            code += [langfmt.array_decl("const double", names.qw, langfmt.array_access(names.weights, ipoint))]

            for r in det_restrictions:
                det = names.det + names.restriction_postfix[r]
                code += [langfmt.array_decl("const double", names.D, langfmt.product(names.qw, det))]
    ...
    def define_argument_loop_vars(self, argument_count):
        ...
        if code:
            comment = langfmt.comment("Compute mapped derivatives of argument %d" % (argument_count,))
            code = [comment] + code + [""]
        return code
