...
def compile_expression_body(expr, language_formatter, statement_formatter):
    ...
    argument_mapping = dict((k,v) for (k,v) in
                            language_formatter._dependency_handler.form_argument_mapping.items()
...
def compile_form(form, prefix=""):
    ...
    for ida in formdata.integral_data:
        for integral in ida.integrals:
            ...
            form_argument_mapping =
            dependency_handler = DependencyHandler(formdata.object_names)

            # This formatter is a multifunction implementing target specific formatting rules
            language_formatter = CppLanguageFormatter(dependency_handler, formdata.function_replace_map)

            # TODO: Also needs to know about integration context:
            statement_formatter = CppStatementFormatter(dependency_handler, formdata.cell)
