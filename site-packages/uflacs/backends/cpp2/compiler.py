...
# TODO: Refactoring!
def compute_expression_body_ir(expr, argument_mapping, integrate):
...
def compile_expression_body(expr, target_formatter, integrate=False):
    ...
    argument_mapping = dict((k,v) for (k,v) in
                            target_formatter.form_argument_mapping.items()
                            if isinstance(k, Argument))
    ir = compute_expression_body_ir(expr, argument_mapping, integrate)
    code = generate_expression_body_code(ir, target_formatter)
    return code

def generate_expression_body_code(ir, target_formatter):
    # This formatter is a multifunction with single operator
    # formatting rules for generic C++ formatting
    target_formatter.update_terminals(ir["terminals"])
