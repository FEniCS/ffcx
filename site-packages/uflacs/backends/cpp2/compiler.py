...
def generate_code_from_ssa(SV, active, partitions, allocations, expr_formatter):
        ...
        for i, v in enumerate(SV):
            ...
            if vreg >= 0:
                ...
                if isinstance(v, terminalish):
                    # FIXME: This can all be handled through the expr_formatter, but we need to renumber coefficients in target formatter first!
                    vcode = str(v)
                else:
    ...
    final_variable_names = [] # FIXME
    return partition_codes, final_variable_names

def compile_expression_code(expression, target_formatter=None):
    ...
    tic('generating code')
    partition_codes, final_variable_names = generate_code_from_ssa(SV, active, partitions, allocations, expr_formatter)

    # FIXME: Temporary hack, should join partitions with target specific loop code
    code = []
    ...
    # so we can assign their values to the target specific output variables
    #target_variable_names, final_variable_names
