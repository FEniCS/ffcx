...
def compile_expression_body(expr, target_formatter, integrate=False):
    ...
    # FIXME: In default_partition_seed, there is no distinction between test/trial.
    rank = len(tfmt.arguments)
    for p in range(3,3+rank):
        ac = 2+rank-p # Magic formula for test/trial vs partition numbering...

        # --- Partitions 3...3+rank-1: trial,test,... function dependency
        loops += [tfmt.define_argument_for_loop(ac)]
        definitions += [tfmt.define_argument_loop_vars(ac)]
        partitions += [partition_codes.get(p,"")]
        if p in partition_codes:
            del partition_codes[p]
...
def compile_expression(expr, prefix):
    ...
    code = compile_expression_body(expr, target_formatter)
