...
def compile_expression_partitions(expression, target_formatter):
    ...
    argument_mapping = dict((k,v) for (k,v) in target_formatter.form_argument_mapping.items()
    ...
    return num_registers, SV, active, partitions, allocations, target_registers

# TODO: Refactoring!
def compile_expression_body_ir(expr, target_formatter, integrate):
    # Compile expression into partitioned code listings
    num_registers, SV, active, partitions, allocations, target_registers =\
                     compile_expression_partitions(expr, target_formatter)
    # TODO: Instead of attaching data to target_formatter,
    #       return data from compile_expression_partitions
    #       and attach to ir!
    ir = {}
    ...
    ir["integrate"] = integrate
    ir["target_formatter"] = target_formatter # TODO: Remove, see above
    return ir

def compile_expression_body(expr, target_formatter, integrate=False):
    ir = compile_expression_body_ir(expr, target_formatter, integrate)
    code = generate_expression_body_code(ir)
    return code

def generate_expression_body_code(ir):
    target_formatter = ir["target_formatter"]
    ...
    partition_codes, final_variable_names = generate_code_from_ssa(
        ...
        ir["target_registers"],
        expr_formatter)
    code = generate_expression_body(target_formatter,
                                    partition_codes,
...
def generate_expression_body(target_formatter, partition_codes,
    ...
    if 1:
        ...
        if integrate:
            scaling_factor = tfmt.integration_scaling_factor()
...
def compile_expression(expr, prefix=""):
    ...
    code = compile_expression_body(expr,
                                   target_formatter,
                                   integrate=False)
...
def compile_form(form, prefix=""):
    ...
    # Preprocess form
    tt.step('preprocess')
    formdata = form.compute_form_data()
    ...
    k = 0
    for ida in formdata.integral_data:
        for integral in ida.integrals:
            ...
            integral_code = compile_expression_body(integrand,
                                                    target_formatter,
                                                    integrate=True)
