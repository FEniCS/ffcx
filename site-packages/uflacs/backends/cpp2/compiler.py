...
def generate_code_from_ssa(SV, active, partitions, allocations,
    ...
    partition_codes = []
    def format_register_variable(r): # TODO: Move to [target_]expr_formatter?
        return "s[%d]" % (r,)
    ...
    for p in range(min_p, max_p+1):
        ...
        for i, v in enumerate(SV):
            ...
            if vreg >= 0:
                ...
                # Format expression
                if 0 and isinstance(v, terminalish):
                    # FIXME: This can all be handled through the expr_formatter,
                    # but we need to renumber coefficients in target formatter first!
                    vcode = str(v)
                else:
                ...
                vname = format_register_variable(vreg)
    ...
    final_variable_names = [format_register_variable(r) for r in target_registers]
    return partition_codes, final_variable_names

def compile_expression_code(expression):
    """Example expression compiler without target specific code generation."""
    ...
    partition_codes, final_variable_names, num_registers = \
                     compile_expression_partitions(expression, target_formatter)

    # Join code, with no target specific variables here because we have no target!
    code = []
    code.append('')
    code.append("double s[%d]" % (num_registers,))
    for p,c in partition_codes:
        code.append('')
        code.append("// Code for partition %d" % (p,))
        code.extend(c)
    code.append('')
    code.append("// Final variables are:")
    code.extend("// %s" % (', '.join(final_variable_names)))
...
def compile_expression_partitions(expression, target_formatter):
    ...
    max_registers = 2*1024 # 16 KB # TODO: Make this a parameter
    score_threshold = 2 # TODO: Make this a parameter
