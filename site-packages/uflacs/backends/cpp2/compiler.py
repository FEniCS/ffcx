...
def compile_expression_body(expr, target_formatter, integrate=False):
    ...
    p = 3
    if p in partition_codes: # FIXME: Use only when needed, is this line good?
        loops += ["for (trial functions)"] # FIXME
        definitions += ["// trial defs"] # FIXME
        partitions += [partition_codes.get(p,"")]
        del partition_codes[p]
    ...
    p = 4
    if p in partition_codes: # FIXME: Use only when needed, is this line good?
        loops += ["for (test functions)"] # FIXME
        definitions += ["// test defs"] # FIXME
        partitions += [partition_codes.get(p,"")]
        del partition_codes[p]
