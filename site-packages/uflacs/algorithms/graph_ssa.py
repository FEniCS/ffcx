...
from ufl.classes import (Terminal, GeometricQuantity,
                         FormArgument, Argument, Coefficient,
...
def coefficient_is_piecewise_constant(t): # FIXME: Move this into UFL
...
def geometry_is_piecewise_constant(t): # FIXME: Move this into UFL
    assert isinstance(t, GeometricQuantity)
    if isinstance(t, (FacetNormal, FacetArea)):
        ...
        error("Unknown geometry object %s" % (expr,))

def default_partition_seed(expr, argument_mapping):
    """
    ...
    """
    rank = len(argument_mapping)

    modifiers = (Grad, Restricted, Indexed)
    if isinstance(expr, modifiers):
        return default_partition_seed(expr.operands()[0], argument_mapping)

    elif isinstance(expr, Argument):
        ac = argument_mapping.get(expr,expr).count()
        assert ac >= 0
        return 2 + rank - ac # Magical formula for mapping argument count to partition

    elif isinstance(expr, Coefficient):
        if coefficient_is_piecewise_constant(expr):
            return 0
        else:
            return 2

    elif isinstance(expr, GeometricQuantity):
        if geometry_is_piecewise_constant(expr):
            return 0
        else:
            return 1
...
def mark_partitions(V, active, dependencies, argument_mapping,
    ...
    for i, v in enumerate(V):
        ...
        if active[i]:
            if len(deps):
                ...
                p = partition_seed(v, argument_mapping)
        else:
            p = -1
        partitions[i] = p
    return partitions
