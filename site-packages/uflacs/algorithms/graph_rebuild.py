...
from uflacs.algorithms.indexing import indexing_to_component
...
class ReconstructScalarSubexpressions(MultiFunction):
    ...
    def element_wise2(self, scalar_operator, o, ops):
        ...
        if sh == ():
            assert all(op.shape() == () for op in o.operands())
        else:
            shaped = 0
    ...
    def index_sum(self, o, ops):
        ...
        assert len(ss) == predim*postdim*d
        sops = []
...
def rebuild_scalar_e2i(G, DEBUG=False):
    ...
    - terminals - Set of (possibly non-scalar) terminal expressions
                  and grad and restrictions of terminals
    """
    ...
    assert all(x is None for x in NV[len(ne2i):])
    NV = NV[:len(ne2i)]
