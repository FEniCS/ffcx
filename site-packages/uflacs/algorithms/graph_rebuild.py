...
class ReconstructScalarSubexpressions(MultiFunction):
    ...
    def index_sum(self, o, ops):
        a, b = o.operands()
        d = a.index_dimensions()[b[0]]

        e, mi = ops
        n = len(e) // d

        # Group scalar subcomponents of a for each scalar output component of o
        sops = [[] for k in xrange(n)]

        error("Not implemented!")
        """FIXME:
        Map each flattened total component of a to
        flattened total component of o by removing
        axis corresponding to b.
        """
        #for k in xrange(n):
        #    sops[k] = [j]

        # For each scalar output component, sum over collected subcomponents
        res = [sum(sop) for sop in sops]
        assert len(res) == n
        return res
