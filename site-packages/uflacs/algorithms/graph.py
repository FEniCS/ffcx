...
from ufl.classes import (Terminal, FormArgument, Grad, Restricted,
                         Indexed, ComponentTensor, ListTensor,
                         Transposed, Variable, UtilityType)
...
from uflacs.algorithms.datastructures import *
...
def build_node_shapes(V):
    ...
    for i,v in enumerate(V):
        ...
        k += len(tsh)
    # Make a more memory efficient CRS representation
    V_shapes2 = CRS(nv, k, int)
    for tsh in V_shapes:
        V_shapes2.push_row(tsh)
    return V_shapes2
...
def get_node_symbols2(expr, e2i, V_symbols):
    assert isinstance(V_symbols, CRS)
...
def build_node_symbols(V, e2i, V_sizes):
    # Sparse int matrix for storing variable number of entries (symbols) per row (vertex).
    symbol_type = int
    V_symbols = CRS(len(V), sum(V_sizes), symbol_type)
...
def build_graph_vertices(G, expr, DEBUG=False):
    if DEBUG:
        print "In build_graph_vertices:"
        print "expr = ", str(expr)
...
def build_graph_symbols(G, expr, DEBUG):
    ...
    G.V_sizes = build_node_sizes(G.V_shapes)
    if DEBUG:
        print "\nV_sizes", G.V_sizes
    ...
    G.V_symbols2, G.total_unique_symbols = build_node_symbols(G.V, G.e2i, G.V_sizes)
    G.V_symbols = G.V_symbols2.data # FIXME: Use crs everywhere
    if DEBUG:
        print "\nsymbols:"
...
def build_graph(expr, DEBUG=False):
    ...
    # Make empty graph
    G = Graph2()
    # Populate vertices, symbols. TODO: What more information to generate?
    build_graph_vertices(G, expr, DEBUG)
    build_graph_symbols(G, expr, DEBUG)
    return G
