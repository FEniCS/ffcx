...
def map_tensor_components(tensor, indexed, multiindex):
    ...
    #print str(e2)
    #print tsh1, tsh2
    assert (not sh1) and (sh2)
    if len(mi) == len(multiindex):
        assert product(tsh1) == product(tsh2)
        assert (ish1) and (not ish2)
    else:
        assert product(tsh1) <= product(tsh2) # FIXME: Is Indexed with fixed index handled correctly below?
        assert (not ish2) # Not sure about this one
