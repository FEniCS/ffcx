...
def collect_argument_factors(SV, dependencies, arg_indices):
    ...
    The result is a triplet (AV, FV, IM):
      ...
      - An expression graph with all non-argument factors:
    ...
    for i,v in enumerate(SV):
        ...
        if not len(deps):
            ...
            uflacs_assert(len(fac0) == len(fac1), "Expecting equal argument rank terms among summands.")
            ...
            facs = [F[deps[j]] for j in len(deps)]
            if any(facs):
