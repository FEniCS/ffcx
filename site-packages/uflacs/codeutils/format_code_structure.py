...
class ArrayDecl(Code):
    ...
    def format(self, level, indentchar, keywords):
        ...
        if self.values is None:
            ...
            if r == 1:
                valuescode = tuple(build_value_list(self.values))
            elif r == 2:
                assert len(self.values[0]) == self.sizes[1]
                inner = []
                for i0 in range(self.sizes[0]):
                    inner.append( tuple(build_value_list(self.values[i0])) )
                ...
                for i0 in range(self.sizes[0]):
                    ...
                    for i1 in range(self.sizes[1]):
                        inner.append( tuple(build_value_list(self.values[i0][i1])) )
                    #outer.append(Indented(build_value_list(inner)))
                    #outer.append(tuple(build_value_list(inner)))
                    outer.append(Indented(["{", build_separated_list(inner, ","), "}"]))
                #valuescode = tuple(build_value_list(outer))
