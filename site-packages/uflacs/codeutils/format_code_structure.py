...
class WithKeywords(Code):
    ...
    def format(self, level, indentchar, keywords):
        return format_code_structure(self.code, level, indentchar, self.keywords)
...
class Class(Code):
    def __init__(self, name, superclass=None, public_body=None, protected_body=None, private_body=None, template_arguments=None, template_multiline=False):
        self.name = name
    ...
    def format(self, level, indentchar, keywords):
        code = []
        if self.template_arguments:
            code += [('template', TemplateArgumentList(self.template_arguments, self.template_multiline))]
        if self.superclass:
