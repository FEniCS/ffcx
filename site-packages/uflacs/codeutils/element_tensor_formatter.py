...
def generate_tt_body(sfmt):

    piecewise_constant_stuff = ["// constant stuff"]
    q_rules = [1]

    code = []
    code.append(piecewise_constant_stuff)
    for q in q_rules:
        qloop = "for ..."
        qloop_body = generate_qloop_body(sfmt, q)
        code.append(qloop)
        code.append(Block(qloop_body))
            
    return code

def generate_qloop_body(sfmt, q):
    code = []
    qheader = "xq..."
    code.append(qheader)
    for b,e in dof_ranges:
        dofloop = generate_dofloop_body(sfmt, q)
        code.append(dofloop)
    return code

def generate_dofloop_body(sfmt, q):
    code = []
    for b,e in dof_ranges[
        pass
    return code
