...
def generate_jacobian_snippets(cell, restriction):
    ...
    decl = [
        'double J%s[%d*%d];' % (r, gd, td),
        ]
    comp = [
        'compute_jacobian_%s_%dd(J%s, vertex_coordinates%s);' % (cell.cellname(), gd, r, r),
        ]
    return decl + comp
...
def generate_jacobian_inverse_snippets(cell, restriction):
    ...
    decl = [
        'double detJ%s;' % r,
        'double K%s[%d*%d];' % (r, gd, td),
        ]
    comp = [
        'compute_jacobian_inverse_%s_%dd(K%s, detJ%s, J%s);' % (cell.cellname(), gd, r, r, r),
        ]
    return decl + comp

# TODO: Split determinant computation into separate codes:
def new_generate_jacobian_determinant_snippets(cell, restriction):
    r = restriction
    gd = cell.geometric_dimension()
    td = cell.topological_dimension()
    ...
    decl = [
        'double detJ%s;' % r,
        'double subdet%s[%d]' % (r, num_subdets),
        ]
    comp = [
        'compute_jacobian_determinant_%s_%dd(subdet%s, detJ%s, J%s);' % (cell.cellname(), gd, r, r, r)
        ]
    return decl + comp

def new_generate_jacobian_inverse_snippets(cell, restriction):
    r = restriction
    gd = cell.geometric_dimension()
    td = cell.topological_dimension()
    decl = [
    ...
    comp = [
        'compute_jacobian_inverse_%s_%dd(K%s, subdets%s, detJ%s, J%s);' % (cell.cellname(), gd, r, r, r, r)
        ]
    return decl + comp

def generate_cell_scaling_factor_snippets(cell):
    decl = [
        'double det = std::fabs(detJ);',
        ]
    return decl
...
def generate_facet_scaling_factor_snippets(cell, restriction):
    ...
    comp = [
        'compute_facet_scaling_factor_%s_%dd(det, vertex_coordinates%s, facet%s);' % (cell.cellname(), gd, r, r),
        ]
    return decl + comp
