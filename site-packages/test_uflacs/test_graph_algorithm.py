...
class GraphAlgorithmTest(UflTestCase):
    ...
    def test_map_list_tensor_symbols(self):
        ...
        s1 = list(get_node_symbols2(A, G.e2i, G.V_symbols2))
        s2 = [get_node_symbols2(e, G.e2i, G.V_symbols2)[0] for e in (u+1,u+2,u+3,u**2+1,u**2+2,u**2+3)]
    ...
    def test_map_transposed_symbols(self):
        ...
        G = build_graph(A, DEBUG=False)
        s1 = list(get_node_symbols2(A, G.e2i, G.V_symbols2))
        s2 = list(get_node_symbols2(w, G.e2i, G.V_symbols2))
        s2[1], s2[2] = s2[2], s2[1]
        self.assertEqual(s1, s2)
    ...
    def test_graph_algorithm(self):
        ...
        self.assertEqual(G.V_symbols2.num_elements, 1)
        self.assertEqual(G.total_unique_symbols, 1)

        expr = u**2
        G = build_graph(expr, DEBUG=0)
        self.assertEqual(G.V_symbols2.num_elements, 3)
        self.assertEqual(G.total_unique_symbols, 3)

        expr = u**2/2
        G = build_graph(expr, DEBUG=0)
        self.assertEqual(G.V_symbols2.num_elements, 4)
        self.assertEqual(G.total_unique_symbols, 4)

        expr = dot(v,v)/2
        G = build_graph(expr, DEBUG=0)
        self.assertEqual(G.V_symbols2.num_elements, 5)
        self.assertEqual(G.total_unique_symbols, 5)

        # Testing Indexed
        expr = v[i]*v[i]
        G = build_graph(expr, DEBUG=0)
        self.assertEqual(G.V_symbols2.num_elements, 2+2+2+1)
