...
from uflacs.algorithms.graph import map_list_tensor_symbols, map_transposed_symbols, get_node_symbols
...
class GraphAlgorithmTest(UflTestCase):
    ...
    def test_map_list_tensor_symbols(self):
        ...
        s1 = list(get_node_symbols(A, G, G.V_symbols))
        s2 = [get_node_symbols(e, G, G.V_symbols)[0] for e in (u+1,u+2,u+3,u**2+1,u**2+2,u**2+3)]
    ...
    def test_map_transposed_symbols(self):
        ...
        G = build_graph(A, DEBUG=False)
        s1 = list(get_node_symbols(A, G, G.V_symbols))
        s2 = list(get_node_symbols(w, G, G.V_symbols))
        s2[1], s2[2] = s2[2], s2[1]
        self.assertEqual(s1, s2)
    ...
    def test_graph_algorithm(self):
        ...
        G = build_graph(expr, DEBUG=0)
        self.assertEqual(G.total_value_size, 21) # 2+4+4+4 + 4+2+1
