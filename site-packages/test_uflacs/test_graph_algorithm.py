...
def build_node_shapes(V):
    ...
    for i,v in enumerate(V):
        ...
        V_shapes[i] = shape
        V_sizes[i] = nshape
    return V_shapes, V_sizes

def build_node_offsets(V_sizes):
    nv = len(V_sizes)
    ...
    for i,s in enumerate(V_sizes):
        ...
        total_value_size += s
    return V_offsets, total_value_size
...
def build_graph(expr, DEBUG=False): # TODO: Split this into multiple functions
    ...
    # List the nodes by their ordering
    ...
    # Compute the node shapes
    G.V_shapes, G.V_sizes = build_node_shapes(G.V)

    # Compute the offsets of nodes into a flattened scalar array
    G.V_offsets, G.total_value_size = build_node_offsets(G.V_sizes)
    if DEBUG:
        print
