...
class GraphAlgorithmTest(UflTestCase):
    ...
    def test_rebuild_expression_from_graph_basic_tensor_expressions(self):
        ...
        G = build_graph(v1)
        v2 = rebuild_expression_from_graph(G)
        self.assertEqual(as_vector((2*v1[0],2*v1[1])), v2)

        v1 = w+w
        G = build_graph(v1)
        v2 = rebuild_expression_from_graph(G)
        self.assertEqual(as_vector((2*v1[0,0],2*v1[0,1],2*v1[1,0],2*v1[1,1])), v2)

        v1 = u*v
        G = build_graph(v1)
        v2 = rebuild_expression_from_graph(G)
        self.assertEqual(as_vector((u*v[0],u*v[1])), v2)

        v1 = u*w
        G = build_graph(v1)
        v2 = rebuild_expression_from_graph(G)
        self.assertEqual(as_vector((u*v[0,0],u*v[0,1],u*v[1,0],u*v[1,1])), v2)
    ...
    def test_rebuild_expression_from_graph(self):
        ...
        if 0: # Fails because of missing IndexSum handling
