...
class DolfinExpressionCompilerTest(UflTestCase):
    ...
    def _test_dolfin_expression_compilation(self, uexpr, expected_lines, expected_values):
        # Compile expression and check expected compilation output
        compiled_lines = compile_dolfin_expression_body(uexpr)
        self.assertEqual(compiled_lines, expected_lines)

        # Wrap it in a dolfin::Expression class
        shape = uexpr.shape()
        name = "MyExpression_%s" % hash(uexpr)
        ...
        for x, v in expected_values:
            ...
            self.assertEqual(list(u), list(v))
    ...
    def xtest_dolfin_expression_compilation_of_coefficient(self):
        # Define some ufl expression
        V = ufl.FiniteElement("CG", cell2D, 1)
        u = ufl.Coefficient(V)
        uexpr = 2*u

        # Define expected output from compilation
        expected_lines = ['dolfin::array<double> w_0(1);', # FIXME: Something like this
                          'w0.eval(x, w_0);',
                          'double s[1];',
                          's[0] = 2 * w[0];',
                          'values[0] = s[0];']
        ...
        # Define expected evaluation values: [(x,value), (x,value), ...]
        expected_values = [((0.0,0.0), (0.0,0.0)), # FIXME: Set values of u!
                           ((0.6,0.7), (1.2,1.4)),
                           ]
