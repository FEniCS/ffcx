...
class Ufl2DolfinExpressionCompilerTest(UflTestCase):
    ...
    def test_dolfin_expression_compilation_of_algebraic_operators(self):
        ...
        expected_lines = ['double s[1];',
                          'Array<double> v_w0(1);',
                          'w0->eval(v_w0, x);',
                          's[0] = (3.14 * std::pow(v_w0[0], 2) + 2 * v_w0[0]) / 4;',
                          'values[0] = s[0];']
    ...
    def test_dolfin_expression_compilation_of_math_functions(self):
        ...
        # Define expected output from compilation
        ucode = 'v_w0[0]'
        vcode = '0.02 + std::abs(std::cos(v_w0[0])) / 2'
        vref = 's[0]'
        self.maxDiff = None
        funcs = 'std::asin(%(v)s) + (std::acos(%(v)s) + (std::atan(%(v)s) + (std::log(%(v)s) + (std::exp(%(u)s) + (std::sin(%(u)s) + std::tan(%(v)s))))))'
    ...
    def test_dolfin_expression_compilation_of_dot_with_index_notation(self):
        ...
        expected_lines = ['double s[1];',
                          'Array<double> v_w0(2);',
                          'w0->eval(v_w0, x);',
                          's[0] = std::pow(%(x)s, 2) + std::pow(%(y)s, 2);' % {'x':xc, 'y':yc},
                          'values[0] = s[0];']
    ...
    def test_dolfin_expression_compilation_of_index_notation(self):
        ...
        expected_lines = ['double s[6];',
                          'Array<double> v_w0(2);',
                          'w0->eval(v_w0, x);',
                          's[0] = std::pow(%(x)s, 2);' % svars,
                          's[1] = %(x)s * %(y)s;' % svars,
                          's[2] = %(xx)s + %(xy)s;' % svars,
                          's[3] = std::pow(%(y)s, 2);' % svars,
                          's[4] = %(yy)s + %(xy)s;' % svars,
