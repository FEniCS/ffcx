...
class CodeUtilsTest(UflTestCase):
    ...
    def test_format_blocks(self):
        ...
        self.assertEqual(format_code_structure(Indented(["hei", Indented("verden")]), indentchar='z'),
                    "zhei\nzzverden")
        ...
        self.assertEqual(format_code_structure(["for (iq...)", Block("foo(iq);")]),
                    "for (iq...)\n{\n    foo(iq);\n}")
        # Making a do loop
        self.assertEqual(format_code_structure(["iq = 0;", "do", (Block("foo(iq);"), " while (iq < nq);")]),
                    "iq = 0;\ndo\n{\n    foo(iq);\n} while (iq < nq);")

    def test_format_keywords(self):
        # Making a for loop with keywords
        self.assertEqual(format_code_structure(["for (%(i)s=0; %(i)s<n; ++%(i)s)", Block("foo(%(i)s);")], keywords={'i': 'k'}),
                    "for (k=0; k<n; ++k)\n{\n    foo(k);\n}")
        self.assertEqual(format_code_structure(WithKeywords(["for (%(i)s=0; %(i)s<n; ++%(i)s)", Block("foo(%(i)s);")],
                                             keywords={'i': 'k'})),
                    "for (k=0; k<n; ++k)\n{\n    foo(k);\n}")
    ...
    def test_format_template_argument_list(self):
        def t(args, mlcode, slcode):
            self.assertEqual(format_code_structure(TemplateArgumentList(args, False), indentchar=' '), slcode)
            self.assertEqual(format_code_structure(TemplateArgumentList(args, True), indentchar=' '), mlcode)
        t(('A',), '<\n A\n>', '<A>')
        t(('A', 'B'), '<\n A,\n B\n>', '<A, B>')
