...
def _interpolate_vertex_values(ufl_element, fiat_element):
    ...
    mappings = fiat_element.mapping()
    ir["needs_jacobian"] = any("piola" in m for m in mappings) or any("pullback as covariant 2-tensor" in m for m in mappings)
...
def needs_oriented_jacobian(fiat_element):
    ...
    # (only contravariant piolas and pullback as covariant 2-tensor seem to need it)
    return ("contravariant piola" in fiat_element.mapping() or
            "pullback as covariant 2-tensor" in fiat_element.mapping())
