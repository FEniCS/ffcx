        ...
        if num_components == 1:
            code += [
                L.ForRange(
                    ...
                    0,
                    num_points,
                    body=L.Assign(physical_values[ip][physical_offset],
            ...
            code += [
                L.ForRange(
                    ...
                    body=L.ForRange(
                        ...
                        num_components,
                        body=L.Assign(
                            physical_values[ip][physical_offset + k],
...
def __ffc_implementation_of__generate_apply_mapping_to_computed_values(L):
    ...
    if mapping == "affine":
        ...
        code += [
            f_const_float(f_tmp_ref(i), f_component(f_values, i + offset))
        ...
        for i in range(gdim):
            # Create Jacobian.
            jacobian_row = [
                f_trans("J", i, j, gdim, tdim, None) for j in range(tdim)
            ]
            # Create inner product and multiply by inverse of Jacobian.
            inner = f_group(f_inner(
                jacobian_row, basis_col))  # sum_j J[i,j], values[offset + j])
        ...
        for i in range(gdim):
            ...
            inv_jacobian_column = [
                f_trans("JINV", j, i, tdim, gdim, None) for j in range(tdim)
            ]
        ...
        for p in range(num_components):
            ...
            value = f_group(
                f_inner([
                    f_inner([
                        f_trans("JINV", j, i, tdim, gdim, None)
                        for j in range(tdim)
        ...
        code += [
            "",
            f_comment(
                "Pullback of a matrix-valued funciton as contravariant 2-tensor mapping values back to the physical element"
            )
        ]
        # Get temporary values before mapping.
        code += [
