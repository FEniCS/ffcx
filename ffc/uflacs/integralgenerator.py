...
from ffc.uflacs.ast_transformations import Vectorizer
...
class IntegralGenerator(object):
    ...
    def generate(self):
        ...
        # Check whether cross-cell vectorization should be performed
        vectorize = self.ir["integrals_metadata"]["cell_batch_size"] > 1
        if vectorize and self.ir["integral_type"] != "cell":
            vectorize = False
            logger.warning("Cross-cell vectorization is currently not implemented for integral types other than 'cell' "
                           "(this is type '{}'). Batching disabled for this integral.".format(self.ir["integral_type"]))

        # Optionally perform cross-cell vectorization of the generated code
        if vectorize:
            vectorizer = Vectorizer(self.ir, self.backend)
            parts = vectorizer.vectorize(parts)
